...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, _oneline.frame.origin.y + [self hLocation:TwoEmpty], 150, LabelHeight)];
        ...
        _numberLabel.text = [NSString stringWithFormat:@"参考价"];
        [self addSubview:_numberLabel];
        _numpayLabel = [[UILabel alloc] initWithFrame:CGRectMake(_numberLabel.frame.origin.x + _numberLabel.frame.size.width, _numberLabel.frame.origin.y, 63, LabelHeight)];
        _numpayLabel.font = [UIFont fontWithName:@"PingFang Light.ttf" size:14];
        _numpayLabel.text = @"元";
        _numpayLabel.adjustsFontSizeToFitWidth = YES;
...
- (void)layoutSubviews {
    ...
    self.messageStringLabel.frame = CGRectMake(20, self.twoLine.frame.origin.y + 20, self.frame.size.width - 40, messagesize.height);
}
