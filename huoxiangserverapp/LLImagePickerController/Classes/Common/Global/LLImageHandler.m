...
#warning mark - xxoo
        //获取视频
//        PHFetchResult <PHAssetCollection *>*uservideo = [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeSmartAlbumVideos options:nil];
//        for (PHAssetCollection *collection in uservideo) {
...
- (void)enumerateAssetsInAssetsGroup:(ALAssetsGroup *)group finishBlock:(void(^)(NSArray <ALAsset *>*result))finishBlock {
    ...
    [group enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
        if (result) {
            // 过滤图片|| [[result valueForProperty:ALAssetPropertyType] isEqual:ALAssetTypeVideo]
...
- (void)enumerateAssetsInAssetCollection:(PHAssetCollection *)collection finishBlock:(void(^)(NSArray <PHAsset *>*result))finishBlock {
    ...
    dispatch_async(self.concurrentQueue, ^{
        // 获取collection这个相册中的所有资源 || obj.mediaType == PHAssetMediaTypeVideo
